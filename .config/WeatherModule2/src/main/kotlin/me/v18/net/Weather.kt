...
class Weather(private val ping: () -> Map<String, JsonElement>) {
    ...
    val status: String?
        get() {
            println((ping.invoke()["weather"]!! as JsonArray)[0].json_map["icon"].toString().substring(1, 3))
            return icon[(ping.invoke()["weather"]!! as JsonArray)[0].json_map["icon"].toString().substring(1, 3)]
        }
